function hfig = GridTapeAlign_220505_20211122dOoceraeaMulti1269_r1084
...
% master path
if ispc
    masterPath = '/Users/davidh/Data/Lee/20211122dOoceraeaMulti1269_r1084/roi_generation';
elseif isunix
    masterPath = '/Users/davidh/Data/Lee/20211122dOoceraeaMulti1269_r1084/roi_generation';
else
...
slot_mask_file = [masterPath filesep 'masks' filesep 'slot_mask.txt'];
%% TODO: Set slot_mask 
section1_mask_file = [masterPath filesep 'masks' filesep 'cra9.txt'];
section2_mask_file = [masterPath filesep 'masks' filesep 'cra1.txt'];
%% TODO: Set section_mask
focus_mask_file = [masterPath filesep 'masks' filesep 'focus_mask.txt'];
%% TODO: Set focus_mask

%%
setappdata(hfig,'slot_mask_file',slot_mask_file);
setappdata(hfig,'section1_mask_file',section1_mask_file);
setappdata(hfig,'section2_mask_file',section2_mask_file);
setappdata(hfig,'focus_mask_file', focus_mask_file);

% output of annotation, in txt
outputPath = [masterPath filesep 'annotations']; % saves annotated relative positions to txt, for each individual section
...
imPath = [masterPath filesep 'slot_images']; % contains images of individual sections | normally imglinks
ParseImageDir(hfig,imPath);

%% Init section
% load first section
disp('Loading first section.')
i_im = 1;
setappdata(hfig,'i_im',i_im);
secID = GetSectionIDfromCounter(hfig,i_im);
disp(['Saving annotations to directory ' outputPath])
S = ScanText_GTA_cra(secID,outputPath, slot_mask_file, section1_mask_file, section2_mask_file, focus_mask_file);
setappdata(hfig,'S',S);
LoadNewMask(hfig,slot_mask_file, section1_mask_file, section2_mask_file, focus_mask_file);
...
% slot_mask_file = [masterPath filesep 'masks' filesep 'slotMask.txt'];
% section1_mask_file = [masterPath filesep 'masks' filesep 'dummySectionMask.txt'];
% section2_mask_file = [masterPath filesep 'masks' filesep 'dummySectionMask.txt'];
% focus_mask_file = [masterPath filesep 'masks' filesep 'focus_mask.txt'];
...
[FileName2,PathName] = uigetfile('*.txt','Select the txt file for section1 mask');
section1_mask_file = fullfile(PathName,FileName2);
[FileName3,PathName] = uigetfile('*.txt','Select the txt file for section2 mask');
section2_mask_file = fullfile(PathName,FileName3);
[FileName4,PathName] = uigetfile('*.txt','Select the txt file for focus mask');
focus_mask_file = fullfile(PathName,FileName4);
...
section1_mask_file = getappdata(hfig,'section1_mask_file');
section2_mask_file = getappdata(hfig,'section2_mask_file');
...
disp('Creating new section mask.')
disp('  Click points around tissue of interest. Double click to finalize position.')

if masktypeID == 2 || masktypeID == 3
    h = impoly;
    setColor(h,[1 1 1]);
    wait(h); % double click to finalize position!
    % update finalized polygon in red color
    ...
    [file,path] = uiputfile(sprintf('section%s_mask.txt', num2str(masktypeID-1)),'Save file name');
    
    % write to file
    f = fullfile(path,file);
    setappdata(hfig,sprintf('section%s_mask_file',num2str(masktypeID-1)),f);
    ...
    disp('Before creating a section mask, use spacebar to select section mask 1 or 2.')
end
end
...
S.section1.translation = GetCenterPos(M(2).pos) - GetCenterPos(M(2).pos_init);
S.section2.translation = GetCenterPos(M(3).pos) - GetCenterPos(M(3).pos_init);
S.focus.translation = GetCenterPos(M(4).pos) - GetCenterPos(M(4).pos_init);
setappdata(hfig,'S',S);

% write to text file
WriteToText_GTA_cra(secID,S,M,outputPath);
end
...
section2_mask_file = getappdata(hfig,'section2_mask_file');
focus_mask_file = getappdata(hfig,'focus_mask_file');
[S,tf] = ScanText_GTA_cra(secID, outputPath, slot_mask_file, section1_mask_file, section2_mask_file, focus_mask_file);
setappdata(hfig,'S',S);
...
%imshow(histeq(im_raw(:,:,1),20),jet(255));
...
if tf % (if annotation file exists)
    ...
    pos_slot_init = dlmread(slot_mask_file,' ',1,0);
    pos_section1_init = dlmread(section1_mask_file,' ',1,0);
    pos_section2_init = dlmread(section2_mask_file,' ',1,0);
    ...
    masktypeID = 2;
    M(masktypeID).pos_init = pos_section1_init;
    
    masktypeID = 3;
    M(masktypeID).pos_init = pos_section2_init;
    
    masktypeID = 4;
    M(masktypeID).pos_init = pos_focus_init;
    
    pos_slot = S.slot.vertices;
    pos_section1 = S.section1.vertices;
    pos_section2 = S.section2.vertices;
    pos_focus = pos_focus_init; %S.focus.vertices;
    
    M(1).pos = pos_slot;
    M(2).pos = pos_section1;
    M(3).pos = pos_section2;
    M(4).pos = pos_focus;
...
% toggle masktypeID
if masktypeID_old < 4
    masktypeID = masktypeID_old + 1;
elseif masktypeID_old == 4
    masktypeID = 1;
end

switch masktypeID
    case 1
        disp('  Selected slot mask.')
    case 2
        disp('  Selected section mask 1.')
    case 3
        disp('  Selected section mask 2.')
    case 4
        disp('  Selected focus mask.')
end
...
elseif masktypeID == 2
    S.section1.rotation = S.section1.rotation + rotationAngle;
elseif masktypeID == 3
    S.section2.rotation = S.section2.rotation + rotationAngle;
end
...
function [pos_slot,pos_section1,pos_section2] = ReconstitutePos(S,M)
% slot
masktypeID = 1;
% 
...
masktypeID = 2;
% rotate
rotationAngle = S.section1.rotation;
pos = M(masktypeID).pos_init;
center = GetCenterPos(pos);
poscenter = repmat(center,size(pos,1),1);
rotationArray = [cos(rotationAngle), -sin(rotationAngle); sin(rotationAngle), cos(rotationAngle)];
pos_section1 = (pos-poscenter) * rotationArray + poscenter;
%     translate1 = pos2(1,:)-pos(1,:);

% translate
translationArray = S.section1.translation;% - translate1;
pos_section1(:,1) = pos_section1(:,1)+translationArray(1);
pos_section1(:,2) = pos_section1(:,2)+translationArray(2);

% section
masktypeID = 3;
% rotate
rotationAngle = S.section2.rotation;
pos = M(masktypeID).pos_init;
center = GetCenterPos(pos);
poscenter = repmat(center,size(pos,1),1);
rotationArray = [cos(rotationAngle), -sin(rotationAngle); sin(rotationAngle), cos(rotationAngle)];
pos_section2 = (pos-poscenter) * rotationArray + poscenter;
%     translate1 = pos2(1,:)-pos(1,:);

% translate
translationArray = S.section2.translation;% - translate1;
pos_section2(:,1) = pos_section2(:,1)+translationArray(1);
pos_section2(:,2) = pos_section2(:,2)+translationArray(2);

end

function LoadNewMask(hfig,slot_mask_file,section1_mask_file, section2_mask_file, focus_mask_file)
%S = getappdata(hfig,'S');
...
pos_section1 = pos_section1_init;
pos_section2 = pos_section2_init;
