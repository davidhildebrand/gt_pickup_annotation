...
write_json = 1;
plot_imgs = 1;
sectionList = startSectionID:1:endSectionID;
sectionList = setdiff(sectionList,skipList,'stable');

%% Set paths and load mask and image
masterPath = '/n/groups/htem/temcagt/datasets/211122dOoceraeaMulti1269_r1084/roi_generation/';
%masterPath = '/Users/davidh/Data/Lee/20211122dOoceraeaMulti1269_reel1084/roi_generation/';

dt = [datestr(now, 'yyyymmdd') 'd' datestr(now, 'HHMMSS') 't'];
queue_output = [masterPath '/queues/' dt '_cra9_' num2str(startSectionID) '-' num2str(endSectionID) '.json'];
...
% image folder
imPath = [masterPath '/slot_images']; % contains images of individual sections
...
problems = sectionList(problematic==1);
unverified = sectionList(verified==0);
...
for i = 1:length(sectionList)
    ...
    idx3 = find(strcmp(s{1}, 'SECTION1'), 1, 'first');
    idx4 = find(strcmp(s{1}, 'NOITCES1'), 1, 'first');
...
%     angle = xyTH(3); % note this is not accurate!
    ...
    idx7 = find(strcmp(s{1}, 'FLAGS'), 1, 'first');
    flags = dlmread(f,'',[idx7+1 0 idx7+1 1]);
...
if sectionList(i) == endSectionID
    ...
    if plot_imgs == 1
        ...
        img_save_path = [masterPath '/slot_images_annotated/' num2str(sectionList(i)) '_cra9.png'];
