...
queue_output = [masterPath '/queues/' date '_' num2str(startSectionID) '-' num2str(endSectionID) 'test.json'];

% saved mask templates for slot and section, respectively, in txt
slot_mask_file = [masterPath '/masks/' 'slotMask20190219.txt'];
section_mask_file = [masterPath '/masks/' 'section_mask_190109_sect496.txt']; %section_mask_190109_sect496.txt
...
section_mask_ref_file = [masterPath '/annotations/' '496.txt'];
ROI_mask_file = [masterPath '/masks/' 'roi_mask_190110_sect496.txt'];
%ROI_mask_file = [masterPath '/masks/' 'calibration.txt'];
...
section_mask_calc = section_mask_orig;
...
%     angle = xyTH(3); % note this is not accurate!
    ...
    % find edges of slot in units of pixels, using 20 vertices
    xL = min(slot(:,1));
    xR = max(slot(:,1));
    yT = max(slot(:,2));
    yB = min(slot(:,2));
