...
fprintf(fileID,'SECTION1\n');
for i = 1:size(M(2).pos,1)
    formatSpec = '%4.2f %4.2f\n';
    fprintf(fileID,formatSpec,M(2).pos(i,1),M(2).pos(i,2));
end
fprintf(fileID,'NOITCES1\n');
fprintf(fileID,'\n');

fprintf(fileID,'SECTION1COM(x,y,theta):\n');
% fprintf(fileID,'section mask\n');
formatSpec = '%4.2f %4.2f %4.2f\n';
%formatSpec = '[section] x: %4.2f; y: %4.2f; theta: %4.2f \n';
fprintf(fileID,formatSpec,S.section1.translation(1),S.section1.translation(2),S.section1.rotation);
fprintf(fileID,'\n');

fprintf(fileID,'SECTION2\n');
for i = 1:size(M(3).pos,1)
    formatSpec = '%4.2f %4.2f\n';
    fprintf(fileID,formatSpec,M(3).pos(i,1),M(3).pos(i,2));
end
fprintf(fileID,'NOITCES2\n');
fprintf(fileID,'\n');

fprintf(fileID,'SECTION2COM(x,y,theta):\n');
% fprintf(fileID,'section mask\n');
formatSpec = '%4.2f %4.2f %4.2f\n';
%formatSpec = '[section] x: %4.2f; y: %4.2f; theta: %4.2f \n';
fprintf(fileID,formatSpec,S.section2.translation(1),S.section2.translation(2),S.section2.rotation);
fprintf(fileID,'\n');

[~,name,ext] = fileparts(S.slot_mask_file);
fprintf(fileID,['slot mask file: ',name,ext,'\n']);
[~,name,ext] = fileparts(S.section1_mask_file);
fprintf(fileID,['section1 mask file: ',name,ext,'\n']);
[~,name,ext] = fileparts(S.section2_mask_file);
fprintf(fileID,['section2 mask file: ',name,ext,'\n']);
[~,name,ext] = fileparts(S.focus_mask_file);
fprintf(fileID,['focus mask file: ',name,ext,'\n']);
fprintf(fileID,'\n');
% close file
fclose(fileID);
